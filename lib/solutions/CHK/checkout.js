...
module.exports = function(skus) {
  ...
  numberOfM = Math.max(0, numberOfM - freeMsFromNSpecialDeal);
  ...
  const priceOfHs =
    ...
    numberOfHSpecialDeals5Bought * 45;
  ...
  const priceOfKs = numberOfKSpecialDeals * 120 + remainingKs * 70;
  ...
  numberOfU = numberOfU - Math.floor(numberOfU / 4);
  ...
  const calculateSTXYZPrice = () => {
    let totalPrice = 0;
    const quantities = [numberOfS, numberOfT, numberOfX, numberOfY, numberOfZ];
    const numberOfDeals =
      Math.floor((numberOfS + numberOfT + numberOfX + numberOfY + numberOfZ) / 3);
    totalPrice += numberOfDeals * 45;
    for (let i = 0; i < 5; i++) {
      quantities[i] -= numberOfDeals * 3;
    }
    return (
      totalPrice +
      quantities.reduce((acc, qty, index) => {
        let price = [20, 20, 17, 20, 21][index];
        return acc + qty * price;
      }, 0)
    );
  };

  const totalPriceSTXYZ = calculateSTXYZPrice();
